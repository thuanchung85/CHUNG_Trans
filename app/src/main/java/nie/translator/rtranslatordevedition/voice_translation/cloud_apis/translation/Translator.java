...
import nie.translator.rtranslatordevedition.TranslatorAPI;
...
import nie.translator.rtranslatordevedition.voice_translation.cloud_apis.translation.model.TranslateListModel;
import nie.translator.rtranslatordevedition.voice_translation.cloud_apis.translation.model.TranslateListModelMessageModel;
...
public class Translator extends CloudApi {  // separate thread for internet translation
    ...
    public interface TranslateListener extends TranslatorListener {
        void onTranslatedText(String text, String textToTransalte,CustomLocale languageOfText);
    }
    ...
    public void translateMessage(final ConversationMessage conversationMessageToTranslate, final CustomLocale languageOutput, final TranslateMessageListener responseListener) {  // what the thread does
        Thread t = new Thread("messageTranslationPerformer") {
            public void run() {
                ...
                Log.d("TTT translateMessage",languageInput.getLanguage()+languageOutput.getLanguage());
        ...
        new Thread("textTranslationPerformer") {
            public void run() {
                ...
                mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        responseListener.onTranslatedText(translatedText,textToTranslate, outputLanguage);
                    }
                });
    ...
    private void performMessageTranslation(final ConversationMessage conversationMessageToTranslate, final CustomLocale inputLanguage, final CustomLocale outputLanguage, final float cost, final TranslateMessageListener responseListener) {
        new Thread("messageTranslationPerformer") {
            public void run() {
                Log.d("TTT","performMessageTranslation");
                final String text = conversationMessageToTranslate.getPayload().getText();
...
//        }
        ...
        Log.d("TTT","translateSimply");
        Log.d("TTT","inputLanguageCode"+global.getFirstLanguage()+"outputLanguageCode"+global.getSecondLanguage());
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                ...
                translatorApi.translateText(text,inputLanguageCode!= null ? inputLanguageCode:"",outputLanguage.getCode(), new com.squareup.okhttp.Callback() {
                    @Override
                    public void onFailure(Request request, IOException e) {
                        Log.d("TTT onFailure","Fail call API Microsoft" + e.getMessage());
                    }
                    ...
                    public void onResponse(Response response) throws IOException {
                        if(response.isSuccessful()){
                            ...
                            if(inputLanguage == null){
                                TranslateListModel[] listData = gson.fromJson(result, TranslateListModel[].class);

                                // detect and save name language
                                String inputVoice =listData[0].getDetectedLanguage().getLanguage();
                                String outputVoice =listData[0].getTranslations()[0].getTo();
                                global.setLanguageDetectInput(inputVoice);
                                global.setLanguageDetectOutput(outputVoice);

                                completableFuture.complete(listData[0].getTranslations()[0].getText());
                                Log.d("TTT translateSimply result",listData[0].getTranslations()[0].getText());
                                Log.d("TTT translateSimply result",result);
                            }else {
                                TranslateListModelMessageModel[] listData = gson.fromJson(result, TranslateListModelMessageModel[].class);
...
//                                global.setLanguageDetectOutput(outputVoice);
                            ...
                            Log.d("TTT translateSimply result onResponse",response.toString());
                        }
                    }
                });
                Log.d("TTT translateSimply result final",completableFuture.get());
